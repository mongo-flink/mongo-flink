...
public class MongoDynamicTableFactory implements DynamicTableSourceFactory, DynamicTableSinkFactory {
    ...
    private static final ConfigOption<String> CONNECT_STRING =
            ConfigOptions.key("connect_string").stringType().noDefaultValue().withDescription("the mongo connect string");
    private static final ConfigOption<String> DATABASE =
            ConfigOptions.key("database").stringType().noDefaultValue().withDescription("the mongo database name");
    private static final ConfigOption<String> COLLECTION =
            ConfigOptions.key("collection").stringType().noDefaultValue().withDescription("the mongo collection name");
    private static final ConfigOption<Boolean> TRANSACTION_ENABLE =
            ConfigOptions.key("sink.transaction.enable").booleanType().defaultValue(false).withDescription("whether enable transaction");
    private static final ConfigOption<Boolean> FLUSH_ON_CHECKPOINT =
            ConfigOptions.key("sink.flush.on-checkpoint").booleanType().defaultValue(false).withDescription("flush on checkpoint");
    private static final ConfigOption<Integer> FLUSH_SIZE =
            ConfigOptions.key("sink.flush.size").intType().defaultValue(1000).withDescription("flush size");
    private static final ConfigOption<Duration> FLUSH_INTERVAL =
            ConfigOptions.key("sink.flush.interval").durationType()
                    .defaultValue(Duration.of(30_000L, ChronoUnit.MILLIS)).withDescription("flush interval");
    ...
    private void validate(ReadableConfig options) {
        ...
        if (options.get(TRANSACTION_ENABLE)) {
            Preconditions.checkArgument(options.get(FLUSH_ON_CHECKPOINT),
                    "`%s` must be true when the transactional sink is enabled",
                    FLUSH_ON_CHECKPOINT.key());
        }
        Preconditions.checkArgument(options.get(FLUSH_SIZE) > 0,
                "`%s` must be greater than 0",
                FLUSH_SIZE.key());
    }

    private MongoConnectorOptions getMongoSinkOptions(ReadableConfig config, Optional<UniqueConstraint> primaryKey) {
